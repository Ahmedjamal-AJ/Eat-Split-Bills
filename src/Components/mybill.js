    ...
    const splitbillhandler = () => {
      ...
      setsingleuser(false);
        ...
        return(
          ...
          <div style={{display:'flex', justifyContent:"space-between", fontFamily:"cursive"}}>
            <label>💰 Bill value</label>
            <input onChange={(e) => setTotalBill(e.target.value)} style={{width:"80px", padding:"4px", borderRadius:"10px", borderStyle:"none", width:"140px", height:"22px"}} type="Number" />
          </div>
          <div style={{display:'flex', justifyContent:"space-between", fontFamily:"cursive"}}>
            <label>🧍‍♂️ Your expense</label>
            <input onChange={(e) => setMyExpense(e.target.value)}  style={{width:"80px", padding:"4px", borderRadius:"10px", borderStyle:"none", width:"140px", height:"22px"}} type="Number" />
          </div>
          <div style={{display:'flex', justifyContent:"space-between", fontFamily:"cursive",}}>
            <label>👫 {naam}'s expense</label>
            <input onChange={(e) => setUserExpense(e.target.value)}  style={{width:"80px", padding:"4px", borderRadius:"10px", borderStyle:"none", width:"140px", height:"22px"}} type="Number" />
          </div>
          <div style={{display:"flex", alignItems:"center", justifyContent:"space-between" ,fontFamily:"cursive",}}>
            <p>🤑 Who is paying the bill?</p>
            <select onChange={(e) => setPayBill(e.target.value)}  style={{width:"93px" , padding:"6px 4px", borderRadius:"10px", fontFamily:"cursive", borderStyle:"none", width:"100px", height:"30px"}}>
                <option style={{fontSize:"15px", fontFamily:"cursive"}}>You</option>
                <option style={{fontSize:"15px", fontFamily:"cursive"}}>{naam}</option>
            </select>
          </div>
          <div style={{display:"flex", justifyContent:"flex-end"}}>
            <button onClick={() => splitbillhandler()} style={{padding:"5px 20px",borderRadius:"8px",border:"none", fontWeight:"600", backgroundColor:"#fd9e40", cursor:"pointer", width:"100px", height:"30px",fontFamily:"cursive" }}>Split Bill</button>
          </div>
        </div>
        )
     }